...
// QQ music api
function qq_api_url(keyword, callback){
  var data = {
    input: keyword,
    filter: 'name',
    type: 'qq',
  ...
  var data = {
    ...
    referrer: `https://music.liuzhijin.cn/?name=${encodeURI(data.input)}&type=qq`,
    body: `input=${encodeURI(data.input)}&filter=name&type=qq&page=1`,
  }
  if(callback) callback(`https://music.liuzhijin.cn/`, data);
  else return [`https://music.liuzhijin.cn/`, data];
}

function qq_songs(data){
  if(data.code != 200 || !data.data || !data.data.length)
    throw `Q: No search result... QAQ`;
  return data['data'];
}

function qq_link(data, idx = 0){
  return qq_songs(data)[idx]['url'];
}

function qq_name(data, idx = 0){
  return qq_songs(data)[idx]['title'];
}

function qq_singer(data, idx = 0){
  return qq_songs(data)[idx]['author'];
}
// also have pic

function qq_fetch(link, cb){
  if(cb) cb(link);
  else return link;
}
...
function data2info(data, src, idx){
  return {
    src: src,
