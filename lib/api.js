...
const api_endpoint = 'https://keeper-cat.fly.dev'
...
// https://[NO MAINTAINED].com/song/url?id=1373706768
...
function ne_fetch(cb, id){
  $.ajax({
    type: "GET",
    url: `https://[NO MAINTAINED].com/song/url?id=${id}`,
    dataType: 'json',
...
function yt_api_url(keyword, callback){
  //return `${api_endpoint}/yt?term=${keyword}`;
  var url = `${api_endpoint}/kw?term=${keyword}`;
  if(callback){
...
function yt_fetch(cb, url){
  ...
  chrome.storage.sync.get("youtube-api-url", (config)=>{
    ...
    else callback(`${api_endpoint}/lk?url=${url}`);
  });
}
...
var api = {
  ...
  // '易': make_liuzhijin_api('netease', '易'),
...
function set_playing_mode(mtype){
  chrome.storage.sync.set({
    [MUSIC_MODE]: mtype
  });
}

function set_playing_source(mtype){
  chrome.storage.sync.set({
    'music_source': mtype
  });
}

function get_playing_mode(callback){
  chrome.storage.sync.get(MUSIC_MODE,
    cfg => callback(cfg[MUSIC_MODE] || ALBUM_MODE));
}

function get_playing_source(callback){
  chrome.storage.sync.get('music_source',
    cfg => callback(cfg['music_source'] || '易'));
}
...
function add_song(list, song, mute = false, publish = false, pos = -1){
  // song: name, link, singer
  push_value(list, song, null, pos);
...
function swp_song(conf_name, here, there, callback, mute = false, publish = false){
  swp_value(conf_name, here, there, callback,
    (cn, a, b) => chrome.notifications.create({
      type: "basic",
      iconUrl: '/icon.png',
      title: `${cn.toUpperCase()} UPDATE`,
      message: `${song_title(a)}, ${song_title(b)} are swapped in ${cn}`
    }),
    (cn, a, b) => sendTab({
      fn: publish_message,
      args: {
        msg: `${song_title(a)}, ${song_title(b)} are swapped in ${cn}`
      }
    }), mute, publish
  )
}

function mov_song(conf_name, from, to, callback, mute = false, publish = false){
  mov_value(conf_name, from, to, callback,
    (cn, item) => chrome.notifications.create({
      type: "basic",
      iconUrl: '/icon.png',
      title: `${cn.toUpperCase()} UPDATE`,
      message: `${song_title(item)} is moved in ${cn}`
    }),
    (cn, item) => sendTab({
      fn: publish_message,
      args: {
        msg: `${song_title(item)} is moved in ${cn}`
      }
    }), mute, publish
  )
}

function shf_song(conf_name, callback, mute = false, publish = false){
  shf_value(conf_name, callback,
    (cn) => chrome.notifications.create({
      type: "basic",
      iconUrl: '/icon.png',
      title: `${cn.toUpperCase()} UPDATE`,
      message: `${cn} is shuffled`
    }),
    (cn, item) => sendTab({
      fn: publish_message,
      args: {
...
function play_search(get_music, log = console.log, idx){
  get_music((keyword, source, data) => {
    let songs = api[source].songs(data);
...
function add_search(get_music, mute, publish, idx, pos = -1){
  get_music((keyword, source, data) => {
