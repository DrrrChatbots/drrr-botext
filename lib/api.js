...
function add_song(name, link, singer, mute = false, publish = false){
    push_value(
        PLAYLIST,
        {
            name: name,
    ...
    if(publish){
        sendTab({
            fn: publish_message,
            args: { msg: `${name} - ${singer} is added to playlist` }
        })
    }
}

function del_song(idx, callback, mute = false, publish = false){
    chrome.storage.sync.get((config) => {
        ...
        if(list && idx < list.length){
            ...
            if(publish){
                sendTab({
                    fn: publish_message,
                    args: { msg: `${name} - ${singer} is deleted from playlist` }
                })
            }
        }
        else{
