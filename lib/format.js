...
function argfmt(arglist, user, cont, url, callback){
  ...
  if(callback){
    return async_map([], cdr, (s, cb) => fmt_syncasync(s, dict, args, cb),
      (done) => fmt_car_async(car, dict, args, done, callback));
  }
  else{
    return fmt_car_sync(car, dict, args,
      cdr.map(s => fmt_syncasync(s, dict, args)));
  }
}
...
function fmt_car_async(str, dict, args, cdr, callback){
  let m = undefined;
  if(m = str.match(/^\S*\$/)){
    for(let name in argsfuncs){
      if(str.startsWith(name, m[0].length)){
        ...
        if(end){
          fmt_syncasync(str.substring(begp + 1, end), dict, args,
  ...
  console.log("====")
  fmt_syncasync(str, dict, args, (_car) => callback([_car, ...cdr]));
}

function fmt_syncasync(str, dict, args, callback){
  let out = '';
        ...
        if(callback){
          let cont = (parsed) => {
            if(parsed){
              ...
              fmt_syncasync(str.substring(end + 1), dict, args,
                (_out) => callback(out + ret + _out));
            }
          }
          return parse_argfuncs(str, i + 1, dict, args, (parsed) => {
            if(parsed) cont(parsed);
            else fmt_syncasync(str.substring(i + 1), dict, args,
              (_out) => callback(out + cur + _out));
          })
        }
        else {
          let parsed = parse_argfuncs(str, i + 1, dict, args);
          if(parsed){
            let [ret, end] = parsed;
            if(typeof ret === 'object')
              ret = JSON.stringify(ret)
...
function fmt_car_sync(str, dict, args, cdr){
  let m = undefined;
  if(m = str.match(/^\S*\$/)){
    for(let name in argsfuncs){
      if(str.startsWith(name, m[0].length)){
        ...
        let end = find_next_paran(str, begp)
        if(end){
          let arg = fmt_syncasync(str.substring(begp + 1, end), dict, args);
  ...
  return [fmt_syncasync(str, dict, args), ...cdr];
}

function parse_argfuncs(str, idx, dict, args, callback){
  for(let name in argfuncs){
    if(str.startsWith(name, idx)){
      let end = find_next_paran(str, idx + name.length)
      if(end){
        let beg = idx + 1 + name.length;
        if(callback){
          return fmt_syncasync(str.substring(beg, end), dict, args,
            (arg) => callback([argfuncs[name](arg), end]));
        }
        else {
          let arg = fmt_syncasync(str.substring(beg, end), dict, args);
          return [argfuncs[name](arg), end];
        }
      }
      break;
    }
