...
function timefmt(str){
  ...
  const hourfmts = [
    ['HH',  hour => String(hour).padStart(2, '0')      ],
    ['H',   hour => String(hour)                       ],
    ['hh',  hour => String(hour == 24 ?
             24 : hour % 12).padStart(2, '0')          ],
    ['h',   hour => String(hour == 24 ? 24 : hour % 12)],
    ['n',   hour => hour >= 12 ? 'p.m.' : 'a.m.'       ],
    ['午',  hour => hour >= 12 ? '下午' : '上午'       ],
  ]
  for(let i = -12; i < 13; i++){
    for(let fmt of hourfmts){
      let pat = i >= 0 ? `%(\\+)?${i}${fmt[0]}` : `%${i}${fmt[0]}`
      let val = fmt[1]((time.getUTCHours() + i + 24) % 24)
      str = str.replace(
        new RegExp(`(^|[^%])${pat}`, 'g'),
        `$1${val}`);
    }
...
function argfmt(arglist, user, cont, url, callback){
  ...
  var args = (((m)=>m?m.map(unquote) : [])(cont.match(/'.*?'|".*?"|\S+/gu))).map(s => s.replace(new RegExp('\\$', 'g'), '$$$$'))
  var fmts = {
    '\\$url': url.replace(new RegExp('\\$', 'g'), '$$$$'),
    '\\$user': user.replace(new RegExp('\\$', 'g'), '$$$$'),
    '\\$cont': cont.replace(new RegExp('\\$', 'g'), '$$$$'),
    '\\$args': args.length ? cont.substring(args[0].length + 1).trim().replace(new RegExp('\\$', 'g'), '$$$$') : '',
    '\\$\\$': '$'
  };
  args = arglist.map((s)=>
    Object.keys(fmts).reduce(
      (acc, fmt)=> acc.replace(new RegExp(`(^|[^\\$])${fmt}`, 'g'),
        (mat, p1, offset, string) => `${p1}${fmts[fmt]}`
      ),
