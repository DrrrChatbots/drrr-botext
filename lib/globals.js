...
function monitRooms(exit, selfRoomId){
    ...
    chrome.storage.sync.get([RULE_NOTE, 'eager-input'], (config)=>{
        (config[RULE_NOTE] || config['eager-input']) && ajaxRooms(
            function(data){
                Profile = data.profile;
...
function checkTagHook(type, input, room){
    ...
    if(type == 0) return room.roomId == input;
    else if(type == 1) return room.users.some(u=>u.tripcode && u.tripcode.match(new RegExp(input, 'i')));
    else if(type == 2) return room.users.some(u=>u.tripcode && u.name.match(new RegExp(input, 'i')));
    else if(type == 3) return room.name.match(new RegExp(input, 'i'));
}

function tagHook(config, room){
    var type = config['eager-type'];
    var input = config['eager-input'];
    if(checkTagHook(type, input, room)){
...
function findAsList(mode, rooms, roomId, callback, hookTag){
    ...
    var entries = [FRIENDS, HOMES, TRIPCODES, RULE_NOTE];
    if(hookTag) entries = entries.concat(['eager-type', 'eager-input']);
    ...
    chrome.storage.sync.get(entries, (config)=>{
        ...
        for(room of Object.values(rooms)){
            ...
            if(hookTag) tagHook(config, room);
