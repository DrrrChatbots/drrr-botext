...
var settings = {
    ...
    [WHITELIST]: {
        ...
        save_cbk: start_switch(SWITCH_BANLIST, ()=>chrome.sync.set({[BANLIST]:WHITELIST})),
    },
    [BLACKLIST]: {
        validate: (d) => d.split("\n").every(is.regex),
        plain: (d) => d,
        ...
        save_cbk: start_switch(SWITCH_BANLIST, ()=>chrome.sync.set({[BANLIST]:BLACKLIST})),
    },
    [BANABUSE]: {
        validate: (d) => d.split("\n").every(is.regex),
        plain: (d) => d,
...
function isFriends(u, config){
    return config[FRIENDS] ? inRegExps(confg[FRIENDS], u.name) : false;
}
