...
var friends = ['^S\\.', '^Nice', '北極', '北帝', '^音子', '^盤子', '^P$', 'Ryo', '鹿茸', '磷', '呱呱', '失憶菜瓜布', '魚乾', '可賊', '^OWO', '玄黛', '琪露', '美美兔', 'otoko', '穿山甲'];

var state = [];
function findUser(names, rooms, callback, roomId){
...
function findUserAsList(names, rooms, roomId){
    var groups = [];
    for(room of Object.values(rooms)){
        if(room.roomId === roomId) continue;
        var users = [];
        ...
        if(users.length){ groups.push([room, users]); }
    }
    return groups;
}
