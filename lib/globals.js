...
var homes = ['Voiceless', '非日常', 'baka city', '觀測站', '琪露']; //, '\\p{Script=Han}'

var profile = undefined;

var state = [], check_count = 0;

var substraction = (a, b) => (a.filter(n => !b.includes(n)))

var roomGuests = (room, users) => {
...
function monitLounge(names, rooms, exit = false, roomId){
    ...
    state = {}
    update.forEach(([room, users], idx, ary)=>{
        state[room.name] = room.users.map(u=>u.name);
    });
    ...
    //if(state.length) chrome.runtime.sendMessage({ 
    //    notification: {
    //        title: `有人離開啦！`,
    //        msg: state.join('\n'),
    //    } 
