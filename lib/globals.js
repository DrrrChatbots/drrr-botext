...
let ctrl_room = "ctrl_room"
let fetch_room = "fetch_room"
...
function getRoom(succ, err){
  $.ajax({
    ...
    error: function(data) {
      if(data.status == 403){
        console.log("403, call front end api");
        sendTab({ fn: fetch_room, args: { } }, null)
      }
    }
  });
}
...
const getCircularReplacer = () => {
  ...
  return (key, value) => {
    if (typeof value === "object" && value !== null) {
      if (seen.has(value)) {
        return `[Circular ${seen.get(value) || 'this'}]`;
      }
      seen.set(value, key);
    }
