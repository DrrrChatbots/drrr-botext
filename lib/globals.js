...
event_events = [event_me      , event_music   , event_leave   , event_join    , event_newhost , event_msg     , event_dm      , event_dmto    ,/* event_logout  , */event_musicbeg , event_musicend, /*event_timer, event_clock, */event_kick, event_ban, event_unban, event_roll, event_roomprofile, event_roomdesc, event_lounge, "*"]
...
action_actions = [action_name, action_desc, action_msg, action_umsg, action_dm, action_udm, action_kick, action_unban, action_ban, action_banrpt, action_plym, action_addm, action_delm, action_lstm, action_nxtm, action_pndm, action_schm, action_horm, action_ocdr, action_gofr, action_func, action_script]
...
var settings = {
  ...
  [WHITELIST]: {
    ...
    empty_cbk: ()=>chrome.storage.sync.get(BANLIST, (config)=>{ if(config[BANLIST] == WHITELIST) close_switch(SWITCH_BANLIST);}),
  ...
  [BLACKLIST]: {
    ...
    load: (d) => d.split("\n"),
    store: (d) => d.trim().replace(/^\s*[\r\n]|\n^\s*$/gm, ''),
    empty_cbk: ()=>chrome.storage.sync.get(BANLIST, (config)=>{ if(config[BANLIST] == BLACKLIST) close_switch(SWITCH_BANLIST);}),
