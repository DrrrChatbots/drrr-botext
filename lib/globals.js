    ...
    chrome.storage.sync.get(conf_name, (config) => {
        ...
        if(list && list.length){
            ...
            if(target >= 0){
                ...
                if(callback) return callback(true, list);
            }
        }
        if(callback) return callback(false, list);
    });
}
    ...
    if(Profile) callback(Profile)
        ...
        sendTab(
            ...
            () => {
                sendTab(
                    {fn: cache_profile},
                    () => initProfile(callback),
                    callback, undefined, 'https://drrr.com/lounge/*'
                )
            },
            callback
        )
...
function monitLounge(names, rooms, exit = false, roomId){
    ...
    //                current.push(val);
    //                if(!LoungeState.includes(val)){
    //                    users.push(user);
    //                } else LoungeState.splice(LoungeState.indexOf(val), 1);
    //            }
    //        }
    ...
    //if(LoungeState.length) chrome.runtime.sendMessage({ 
    //    notification: {
    //        title: `有人離開啦！`,
    //        msg: LoungeState.join('\n'),
    //    } 
    //});
    //LoungeState = current;
    //console.log('new LoungeState', LoungeState);
}
