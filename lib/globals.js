...
var friends = ['^S\\.', '^Nice', '北極', '北帝', '^音子', '^盤子', '^P$', 'Ryo', '鹿茸', '磷', '呱呱', '失憶菜瓜布', '魚乾', '可賊', '^OWO', '玄黛', '琪露', '美美兔', 'otoko', '穿山甲', 'smile', '桑塔', '繭', '西北風', '123'];

var homes = ['Voiceless', '非日常', 'baka city', '觀測站', '琪露'];
...
function monitLounge(names, rooms, callback, roomId){


    console.log(rooms, names);
    console.log('prev state', state);
    var current = [];
    update = findAsList({'home':true}, rooms, roomId);
        ...
        for(user of room.users){
            for(name of names){
                if(user.name.match(new RegExp(name, 'i'))){
                    var val = `${user.name} 離開了 ${room.name}`;
                    current.push(val);
                    if(!state.includes(val)){
                        users.push(user);
                    } else state.splice(state.indexOf(val), 1);
                }
            }
    ...
    if(state.length) chrome.runtime.sendMessage({ notification: {
        title: `有人離開啦！`,
        msg: state.join('\n'),
    } });
    state = current;
...
function isHome(room){
    for(name of homes)
        if(room.name.match(new RegExp(name, 'i')))
            return true;
    return false;
}
...
function getFriends(room){
    ...
    for(user of room.users){
        for(f of friends){
            if(user.name.match(new RegExp(f, 'i'))){
                users.push(user);
            }
        }
    }
...
function findAsList(mode, rooms, roomId){
    ...
    for(room of Object.values(rooms)){
        if(room.roomId === roomId) continue;
        if(mode['HOME'] && !isHome(room)) continue;

        var users = getFriends(room);
        if(mode['FRIEND'] && !users.length) continue;
