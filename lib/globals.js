...
event_kick = "kick"
event_ban = "ban"
event_banrpt = "report_and_ban_user"
event_unban = "unban"
event_roll = "roll"

event_events = [event_me      , event_music   , event_leave   , event_join    , event_newhost , event_msg     , event_dm      , event_dmto    , event_submit  , event_newtab  , event_exittab , event_exitalarm , event_logout  , event_musicbeg , event_musicend, event_timer, event_kick, event_ban, event_banrpt, event_unban, event_roll]
...
function match_user(name, trip, nameTripRegex){
  ...
  return (nameRegex.length && name.match(new RegExp(nameRegex, 'i')) || (trip && nameTripRegex.includes('#') && trip.match(new RegExp(tripRegex, 'i'))));
}
