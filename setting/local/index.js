...
`
    ...
    $.get(chrome.extension.getURL(module.doc_url), function(data, status){
      ...
      let ViewButtons = id => {
        ...
        return `
          ...
          <!-- TODO complete form view settings
      ...
      let ConfigViews = id => {
        ...
        return `
          ...
          <div class="${id}-cfg-view ${id}-form-cfg-view" ${formViewStyle}>
          <input type="text" placeholder="type">
          <input type="text" placeholder="user">
          <input type="text" placeholder="cont">
          <input type="text" placeholder="func">
  ...
  $(document).ready(() => {
    ...
    let index = window.location.toString().split('#')[1]
                 || localStorage.getItem('last-smenu') || 'menu0';
    ...
    // chrome.storage.local.get(['last-smenu', 'cfg-view'], res => {
    //   let index = window.location.toString().split('#')[1]
    //                || res['last-smenu'] || 'menu0';

    //   let viewType = res['cfg-view'] || 'text'; // form default?

    //   onReady(index, viewType);
    // });
  });
