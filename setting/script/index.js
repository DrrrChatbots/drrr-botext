...
function load_local_modules(installed, index){
  ...
  for(name in installed){
    $('#local-modules').append(
      `<div>
       <label for="${name}"><input class="local-modules" type="checkbox" id="${name}" name="${name}" ${name in installed && installed[name].load[script_mod] ? "checked" : ""}>${name}</label>
     </div>`)
  }
}
...
function preloaded_code(code){
  ...
  for(n in local_modules)
    if(local_modules[n].load[script_mod]) pre += "\n" + local_modules[n].code;
  return pre + code;
}
...
function save_preload_module(){
  for(mod of $('.local-modules')){
    local_modules[mod.name].load[script_mod] = mod.checked;
  }
  chrome.storage.local.set({
    'bs-installed': local_modules
  });
...
$(document).ready(function(event) {
  ...
  chrome.storage.local.get([temp_save, 'bs-mirror', 'bs-mirrors', 'show-chatroom', 'bs-installed'], (config) => {
